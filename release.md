...
* Single-file configuration support via the web page
* `AT_BASE_DIR` environment variable added for easier non-Docker usage
* Environment file support (.env) using python-dotenv for easier setup

### Backstage Improvements

* API key is now obscured when outputting it to the log file
* Threading support for concurrent web and directory operations, which resulted in a lot of minor changes

## Migration Guide

1. **Docker compose**: Update files to include port mapping (e.g., `ports:`) for web access
2. **CLI users**: No changes required; existing functionality is preserved
